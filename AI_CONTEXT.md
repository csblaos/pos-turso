...
- Orders:
  ...
  - UX `/orders/new`:
    ...
    - บล็อก `ชำระด้วย QR โอนเงิน` ในหน้า `/orders/[orderId]` ปรับตาม flow แล้ว:
      - `Walk-in + ชำระแล้ว` แสดงเป็น read-only summary เท่านั้น (รูป QR + ชื่อบัญชี + ธนาคาร + เลขบัญชี) และซ่อน field/action `ลิงก์สลิป...`
      - `Pickup/Online` ที่ยังต้องส่งหลักฐาน จะเห็นช่อง `ลิงก์หลักฐานการชำระ` + ปุ่ม `แนบหลักฐาน / ส่งรอตรวจสอบ`
      - ถ้ามีสลิปแล้วและไม่อยู่ในขั้นกรอกต่อ จะเปลี่ยนเป็นบล็อก read-only `หลักฐานการชำระ` แทนภาษาที่สื่อว่า `รอตรวจสอบ`
