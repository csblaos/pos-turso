...
| Endpoint | Methods | Access Control | Notes |
|---|---|---|---|
| `/api/products` | `GET` | `Permission:products.view` | รายการสินค้าแบบ pagination (`q`,`categoryId`,`status`,`sort`,`page`,`pageSize`) + คืน `total`,`hasMore`,`summary`, ข้อมูล variant (`modelName`,`variantLabel`,`variantOptions`), ค่าสต็อก (`stockOnHand`,`stockReserved`,`stockAvailable`) และ `costTracking` (source/time/actor/reason/reference); route นี้ใช้ current-session permission path แล้ว และตอบ `Cache-Control: no-store` + `Server-Timing` (`auth/db/logic/response/app`) + `count`,`latency` |
