...
| Endpoint | Methods | Access Control | Notes |
|---|---|---|---|
| `/api/orders` | `GET` | `Permission:orders.view` | list orders (รองรับ query `tab`,`page`,`pageSize`; `tab` ใช้ work-queue values `ALL`,`PAYMENT_REVIEW`,`TO_PACK`,`TO_SHIP`,`PICKUP_READY`,`COD_RECONCILE`; response row มี `status` + `paymentStatus` และ page payload มี `queueCounts` สำหรับ count badge ของหน้า `/orders`; UI ใช้แยกเคส `READY_FOR_PICKUP` เป็น badge รอง `ค้างจ่าย`/`ชำระแล้ว`/`รอตรวจสลิป`) |
