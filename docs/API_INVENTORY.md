...
| Endpoint | Methods | Access Control | Notes |
|---|---|---|---|
| `/api/products` | `GET` | `Permission:products.view` | รายการสินค้าแบบ pagination (`q`,`categoryId`,`status`,`sort`,`page`,`pageSize`) + คืน `total`,`hasMore`,`summary`; รอบล่าสุดเป็น `list-lite` path สำหรับหน้า `/products` และ SSR seed: ยังมีข้อมูล variant/category/basic stock (`stockOnHand`,`stockReserved`,`stockAvailable`) แต่ไม่แบก `conversions` และ cost audit detail ทุกแถว; page rows ใช้ raw SQL + join `inventory_balances` โดยตรงเพื่อลด phase ของ read path; route นี้ใช้ current-session permission path แล้ว และตอบ `Cache-Control: no-store` + `Server-Timing` (`auth/db/logic/response/app`) + `count`,`latency` |
...
| `/api/products/[productId]` | `GET` | `Permission:products.view` | full product detail สำหรับ detail/edit/duplicate; คืน `conversions`, `costTracking`, `latestPurchaseOrderCost` และ stock summary ของสินค้ารายนั้น พร้อม `Cache-Control: no-store` + `Server-Timing` |
...
| `/api/products/search` | `GET` | `Permission:products.view` | search (`q`,`includeStock`); รอบล่าสุด route นี้ตั้ง `runtime=nodejs`, `dynamic=force-dynamic`, `preferredRegion=["hnd1","sin1"]`, ตอบ `Cache-Control: no-store` + `Server-Timing` แบบ breakdown (`auth/db/logic/response/app`) และเพิ่ม `count`,`latency` ใน JSON response; เคส `includeStock=true` ไม่ยิง balance query ซ้ำทีละสินค้าแล้ว แต่ map จาก stock fields ที่ query layer โหลดมาอยู่แล้ว |
...
| Endpoint | Methods | Access Control | Notes |
|---|---|---|---|
| `/api/stock/current` | `GET` | `Permission:inventory.view` | stock overview ของ `productId`; route นี้ใช้ raw SQL ผ่าน shared libsql client โดยตรง, ตั้ง `runtime=nodejs`, `dynamic=force-dynamic`, `preferredRegion=["hnd1","sin1"]`, และตอบ `Cache-Control: no-store` + `Server-Timing` แบบ breakdown (`auth/db/logic/response/app`) + `latency`; source ของยอดคงเหลืออ่านจาก `inventory_balances` แล้ว และ auth path ของ current-session จะใช้ role/permission cache ก่อนในเคส active store เดียวกัน |
| `/api/stock/products` | `GET` | `Permission:inventory.view` | stock products (pagination: `page`,`pageSize`; รองรับ filter `categoryId` และ `q` สำหรับค้นหา `sku/name/barcode`); route นี้ตั้ง `runtime=nodejs`, `dynamic=force-dynamic`, `preferredRegion=["hnd1","sin1"]`, ตอบ `Cache-Control: no-store` + `Server-Timing` แบบ breakdown (`auth/db/logic/response/app`), และเพิ่ม `count`,`latency` ใน response; query page หลักลด abstraction โดยใช้ raw SQL + join `inventory_balances` แทน aggregate `inventory_movements` ต่อ request; inventory path จะไม่โหลด `unitOptions` แล้วเพื่อลด latency และ recording path สามารถเรียก exact product แบบ `productId` + `includeUnitOptions=true` เพื่อ lazy-load หน่วยพร้อม stock snapshot ของสินค้าที่เลือกเท่านั้น (exact-product mode นี้ใช้ fast path แยกและรัน query product/balance กับ conversions แบบขนาน) |
| `/api/stock/movements` | `GET` | `Permission:inventory.view` | default: คืน `products + movements` สำหรับ stock overview; รองรับโหมด history (`view=history`) พร้อม query `page`,`pageSize`,`type`,`q`,`productId`,`dateFrom`,`dateTo` เพื่อ list movement แบบ server-side pagination/filter; รอบล่าสุดตอบ `Cache-Control: no-store` + `Server-Timing` แบบ breakdown (`auth/db/logic/response/app`) + `latency` |
...
| `/api/stock/purchase-orders/[poId]` | `GET` | `Permission:inventory.view` | PO detail (item row คืน `unitId`,`purchaseUnitCode`,`multiplierToBase`,`qtyBaseOrdered`,`qtyBaseReceived`,`baseUnitCode`; PO level คืน `shippingCostOriginal/shippingCostCurrency/otherCostOriginal/otherCostCurrency` เพิ่ม) ใช้ current-session auth path, ตอบ `Cache-Control: no-store` + `Server-Timing`, และ payload มี `latency` สำหรับ perf breakdown |
