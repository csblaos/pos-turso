...
- แท็บ `history` ของหน้า `/stock` ขยาย filter ให้รองรับ `barcode` แล้วทั้งฝั่ง UI และ backend: search placeholder/helper ระบุว่าใช้ barcode no. ได้, query `/api/stock/movements?view=history` match `sku + name + barcode`, เพิ่มปุ่มสแกนบาร์โค้ดใน filter row, และเมื่อสแกนเสร็จระบบจะใส่เลข barcode ลง search filter และ apply ทันที; cards ของ movement แสดง barcode แบบ mini text เมื่อมีค่า
- แท็บ `history` ของหน้า `/stock` แยก layout filter เป็น 2 บรรทัดแล้ว: บรรทัดแรก `search + scan`, บรรทัดที่สอง `type + date from + date to` และเอา helper text ใต้ search ออกเพื่อ save area
